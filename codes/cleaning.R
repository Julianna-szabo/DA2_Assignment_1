...
# Second I clean the WDI data

## Check the observations:
#   Lot of grouping observations
#     usually contains a number
d1 <- pop_data %>% filter(grepl("[[:digit:]]", pop_data$iso2c))
d1
# Filter these out
pop_data <- pop_data %>% filter( !grepl("[[:digit:]]", pop_data$iso2c) )
...
drop_id <- c("EU","HK","OE")
# Check for filtering
pop_data %>% filter( grepl( paste( drop_id , collapse="|"), pop_data$iso2c ) ) 
# Save the opposite
pop_data <- pop_data %>% filter( !grepl( paste( drop_id , collapse="|"), pop_data$iso2c ) ) 

# 2nd drop values with certain starting char
# Get the first letter from iso2c
fl_iso2c <- substr(pop_data$iso2c, 1, 1)
retain_id <- c("XK","ZA","ZM","ZW")
# Check
d1 <- pop_data %>% filter( grepl( "X", fl_iso2c ) | grepl( "Z", fl_iso2c ) & 
                       !grepl( paste( retain_id , collapse="|"), pop_data$iso2c ) ) 
# Save observations which are the opposite (use of !)
pop_data <- pop_data %>% filter( !( grepl( "X", fl_iso2c ) | grepl( "Z", fl_iso2c ) & 
                          !grepl( paste( retain_id , collapse="|"), pop_data$iso2c ) ) ) 
...
m <- pop_data %>% filter( !complete.cases( pop_data ) )
# Drop if life-expectancy, gdp or total population missing -> if not complete case except iso2c
pop_data <- pop_data %>% filter( complete.cases( pop_data ) | is.na( pop_data$iso2c ) )
