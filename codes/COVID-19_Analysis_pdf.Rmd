...
```
Alpha says there are on average -3.38 ln(deaths per 10'000 people) in a country because of COVID-19 when there are ln(cases per 10'000 people) is zero.
...
For this hypothesis test, I will use a 95% Confidence Interval.
...
hypothesis <- linearHypothesis( reg4 , "ln_cases_ppc = 0")
hypothesis <- data.frame(model = 1:2, hypothesis)
kable(hypothesis)
```
